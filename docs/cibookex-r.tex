...
\NormalTok{  tabdisp qsmk, cell(}\OtherTok{`var'}\NormalTok{) }\KeywordTok{format}\NormalTok{(%3.1f)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{drop}\NormalTok{ meanY_b}

\KeywordTok{restore}

\KeywordTok{end}

\CommentTok{/* Then we use the `simulate` command to run the bootstraps as many}
...
\NormalTok{  difference = }\FunctionTok{r}\NormalTok{(boot_diff), reps(10) }\DecValTok{seed}\NormalTok{(1): bootstdz}
...
\KeywordTok{mata}\NormalTok{ st_matrix(}\StringTok{"observe"}\NormalTok{, observe)}
...
\KeywordTok{ssc}\NormalTok{ install extremes }
...
\KeywordTok{forvalues}\NormalTok{ i =  }\OtherTok{`seq_start'}\NormalTok{(}\OtherTok{`seq_by'}\NormalTok{)}\OtherTok{`seq_end'}\NormalTok{ \{}
    \KeywordTok{local}\NormalTok{ j = }\OtherTok{`j'}\NormalTok{ + 1}
    \KeywordTok{qui} \KeywordTok{replace}\NormalTok{ psi = }\OtherTok{`i'}
    \KeywordTok{qui} \KeywordTok{replace}\NormalTok{ Hpsi = wt82_71 - psi * qsmk }
...
\NormalTok{      [pw = w_cens], }\KeywordTok{cluster}\NormalTok{(seqn)}
    \FunctionTok{matrix}\NormalTok{ p_mat = }\FunctionTok{r}\NormalTok{(}\KeywordTok{table}\NormalTok{)}
    \FunctionTok{matrix}\NormalTok{ p_mat = p_mat[}\StringTok{"pvalue"}\NormalTok{,}\StringTok{"qsmk:Hpsi"}\NormalTok{]}
    \KeywordTok{local} \KeywordTok{p}\NormalTok{ = p_mat[1,1]}
    \KeywordTok{local}\NormalTok{ b = _b[Hpsi]}
    \KeywordTok{di} \StringTok{"coeff"}\NormalTok{, %6.3f }\OtherTok{`b'}\NormalTok{, }\StringTok{"is generated from psi"}\NormalTok{, %4.1f }\OtherTok{`i'}
    \FunctionTok{matrix}\NormalTok{ results[}\OtherTok{`j'}\NormalTok{,1]= }\OtherTok{`i'}
    \FunctionTok{matrix}\NormalTok{ results[}\OtherTok{`j'}\NormalTok{,2]= }\OtherTok{`b'}
    \FunctionTok{matrix}\NormalTok{ results[}\OtherTok{`j'}\NormalTok{,3]= }\FunctionTok{abs}\NormalTok{(}\OtherTok{`b'}\NormalTok{)}
    \FunctionTok{matrix}\NormalTok{ results[}\OtherTok{`j'}\NormalTok{,4]= }\OtherTok{`p'}
\NormalTok{\}}
...
\KeywordTok{mata}
\NormalTok{res = st_matrix(}\StringTok{"results"}\NormalTok{)}
...
\NormalTok{\}}
\KeywordTok{end}
\NormalTok{* Setting seq_by = 0.01 will yield the result 3.46}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{ssc}\NormalTok{ install tomata}
